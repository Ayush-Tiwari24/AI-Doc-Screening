...
class ScreeningSession(Base):
    ...
    traveler_ref_id: Mapped[str | None] = mapped_column(
        String(100)
    )
...
class ExtractedData(Base):
    ...
    doe: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True)
    )
    ...
    mrz_raw: Mapped[str | None] = mapped_column(
        Text
    )
...
class TamperingResult(Base):
    ...
    technique: Mapped[TamperingTechnique] = mapped_column(
        SAEnum(
            TamperingTechnique,
            name="tampering_technique",
        ),
        nullable=False,
    )

    suspicious_score: Mapped[float | None] = mapped_column(
        Float
    )

    heatmap_path: Mapped[str | None] = mapped_column(
        String(500)
    )
    ...
    document: Mapped["Document"] = relationship(
        back_populates="tampering_results"
    )
...
class FaceVerification(Base):
    ...
    doc_photo_path: Mapped[str | None] = mapped_column(
        String(500)
    )
    ...
    match: Mapped[bool | None] = mapped_column(
        Boolean
    )
